...
RUN apt-get update && \
    ...
    libgoogle-glog-dev libboost-all-dev caffe-cuda libhdf5-dev libatlas-base-dev lsb-release

#for python api
RUN pip3 install --upgrade pip
RUN pip3 install numpy opencv-python --verbose
...
#double install CUDA/cuDNN
#RUN sed -i 's/sudo//g' scripts/ubuntu/install_cuda.sh
#RUN sed -i 's/ubuntu2004.pin/ubuntu1804.pin/g' scripts/ubuntu/install_cuda.sh
#RUN cat scripts/ubuntu/install_cuda.sh
#RUN scripts/ubuntu/install_cuda.sh
#RUN echo ":3"

#get models from altenate link
